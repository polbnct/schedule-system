...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(isset($_POST['login'])){
        ...
        if($loggedInStudent){
            ...
            // --- THIS IS THE FIX ---
            // Save the course_id to the session upon successful login
            $_SESSION['student_course_id'] = $loggedInStudent->course_id;
    ...
    if (isset($_POST['submit_excuse'])) {
        ...
        $data = [
            'student_db_id' => $_SESSION['student_db_id'],
            'course_id'     => $_SESSION['student_course_id'], // This will now work correctly
            'absence_date'  => trim($_POST['absence_date']),
            'reason'        => trim($_POST['reason'])
        ];

        // Basic Validation
        if (empty($data['absence_date']) || empty($data['reason'])) {
            header('Location: ../submit_excuse.php?error=empty');
            exit();
        }

        if ($student->submitExcuseLetter($data)) {
            header('Location: ../my_excuses.php?success=1');
            exit();
        } else {
            header('Location: ../submit_excuse.php?error=failed');
            exit();
        }
    }
}
