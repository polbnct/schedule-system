...
class Student {
    ...
    public function submitExcuseLetter($data) {
    $this->db->query('INSERT INTO excuse_letters (student_db_id, course_id, absence_date, reason) VALUES (:student_db_id, :course_id, :absence_date, :reason)');
    
    // Bind data
    $this->db->bind(':student_db_id', $data['student_db_id']);
    $this->db->bind(':course_id', $data['course_id']);
    $this->db->bind(':absence_date', $data['absence_date']);
    ...
    public function getExcuseLetters($student_db_id) {
    $this->db->query('SELECT absence_date, reason, status, admin_remarks, submitted_at FROM excuse_letters WHERE student_db_id = :student_db_id ORDER BY submitted_at DESC');
    $this->db->bind(':student_db_id', $student_db_id);
